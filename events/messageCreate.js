module.exports = (client, message) => {
    ...
    if (cmd && DJ.enabled && DJ.commands.includes(cmd.name)) {
        ...
        if (!message.member._roles.includes(roleDJ.id)) {
            return message.channel.send(`Questo comando è riservato ${DJ.roleName} al ruolo ${message.author}... Riprova ? ❌`);
        }
    }

    if (cmd && cmd.voiceChannel) {
        if (!message.member.voice.channel) return message.channel.send(`Non sei in un canale vocale ${message.author}... Riprova ? ❌`);
